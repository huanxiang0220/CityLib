package com.mystery.update.service;
...
import com.mystery.update.AndroidOInstallPermissionListener;
import com.mystery.update.AndroidOPermissionActivity;
import com.mystery.update.HttpManager;
import com.mystery.update.R;
import com.mystery.update.UpdateAppBean;
import com.mystery.update.utils.AppUpdateUtils;
...
public class DownloadService extends Service {
    ...
    private String channelName = "This Test upgrade's notification";
...
//    }
    ...
    private void setUpNotification() {
        ...
        Drawable largeICon = AppUpdateUtils.getAppIcon(DownloadService.this);
        if (largeICon == null) largeICon = ContextCompat.getDrawable(this,
                R.mipmap.lib_update_app_update_icon);
//        BitmapFactory.decodeResource(getResources(), R.mipmap.logo)
        ...
        mBuilder.setContentTitle("开始下载")
                ...
                .setLargeIcon(AppUpdateUtils.drawableToBitmap(largeICon))
                .setOngoing(true)
                .setAutoCancel(true)
...
//        六、如果你不想卸载app的话，有个最好的办法就是同时更换channelId和NotificationManager.IMPORTANCE_LOW就可以了。
    ...
    class FileDownloadCallBack implements HttpManager.FileCallback {
        ...
        FileDownloadCallBack(@Nullable DownloadCallback callback) {
            super();
        ...
        public void onError(String error) {
            Toast.makeText(DownloadService.this,
                    "更新新版本出错，" + error, Toast.LENGTH_SHORT).show();
            //App前台运行
            if (mCallBack != null) {
            ...
            try {
                if (AppUpdateUtils.isAppOnForeground(DownloadService.this) || mBuilder == null) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        ...
                        if (b) {
                            AppUpdateUtils.installApp(DownloadService.this, file);//安装应用的逻辑(写自己的就可以)
                        } else {
...
//                                    ToastUtils.shortToast(context, "授权失败，无法安装应用");
                            ...
                            Intent intent1 = new Intent(DownloadService.this, AndroidOPermissionActivity.class);
                            intent1.setFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
                            startActivity(intent1);
                        }
                    } else {
                        boolean install = AppUpdateUtils.installApp(DownloadService.this, file);
                        Log.e("DownloadService", "install:" + install);
                    }
                } else {
                    backstageInstall(file);
                }
                //下载完自杀
                close();
            } catch (Exception e) {
     ...
     * app后台调用安`装
     */
    private void backstageInstall(File file) {
        //App后台运行
        //更新参数,注意flags要使用FLAG_UPDATE_CURRENT
        Intent installAppIntent = AppUpdateUtils.getInstallAppIntent(DownloadService.this, file);
        PendingIntent contentIntent = PendingIntent.getActivity(DownloadService.this,
                0, installAppIntent, PendingIntent.FLAG_UPDATE_CURRENT);
